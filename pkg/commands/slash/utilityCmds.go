...
// /feedback — user feedback intake. Posts a categorized embed to a
// maintainer-visible channel in BuddieBotHQ (no DB storage). Category +
// spec MinLength keep intake quality reasonable; a per-user hourly rate
// limit keeps any one user from dominating the destination channel.

// feedbackLimiter throttles /feedback to one submission per (user, category)
// every 30 minutes. The composite key (userID + ":" + category) means a user
// can submit one of each category in quick succession — feature suggestion
// then bug report then other — but can't spam the same kind.
var feedbackLimiter = helper.NewRateLimiter(30 * time.Minute)
...
func sendFeedbackResponse(s *discordgo.Session, i *discordgo.InteractionCreate, cfg *config.Configs) error {
	...
	// Rate-limit per (user, category) — the key depends on category, so it
	// has to be built AFTER option parsing. Pre-defer ephemeral slot is used
	// for the rejection message.
	if ok, retry := feedbackLimiter.Allow(submitter.ID + ":" + category); !ok {
		return helper.SendEphemeralMsgPreDeferred(s, i,
			fmt.Sprintf("One %s per 30 minutes. Try again in `%s`, or submit a different category.",
				categoryLabel(category), formatRetryAfter(retry)))
	}
